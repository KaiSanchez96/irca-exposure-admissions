############################################################
# irca.R — IRCA project (portfolio visuals rewrite, patched)
...
#### 0.2) Plot helpers ####
theme_port <- function(base_size = 11){
...
add_irca_band <- function(p, xmin = 1989, xmax = 1992){
  p + annotate(
    "rect",
    xmin = xmin, xmax = xmax,
    ymin = -Inf, ymax = Inf,
...
coef_ci_df <- function(model, term, label){
  tt <- broom::tidy(model)
  rr <- tt[tt$term == term, , drop = FALSE]
  stopifnot(nrow(rr) == 1)
  tibble::tibble(
    label = label,
    estimate = rr$estimate,
    std.error = rr$std.error,
    lo = rr$estimate - 1.96 * rr$std.error,
    hi = rr$estimate + 1.96 * rr$std.error
  )
}

extract_irca_post <- function(mod, outcome_label, grp = NULL){
  tt <- broom::tidy(mod)
  rr <- tt[tt$term == "irca_intensity:post", , drop = FALSE]
  if (nrow(rr) == 0) {
    stop(glue("No se encontró el término irca_intensity:post para '{outcome_label}'."))
  }
  rr$outcome <- outcome_label
  if (!is.null(grp)) rr$grp <- grp
  rr
}

#### 0.3) General magnitudes writer ####
...
coef_df <- bind_rows(
  extract_irca_post(m_blue_lvl,      "Blue-collar count", "Levels"),
  extract_irca_post(m_occ_known_lvl, "Known-occupation denominator", "Levels"),
  extract_irca_post(m_occ_total_lvl, "Full denominator", "Levels"),
  extract_irca_post(m_noocc_lvl,     "No-occupation count", "Levels"),
  extract_irca_post(m_ln_blue,       "Blue-collar count", "Logs"),
  extract_irca_post(m_ln_occ_known,  "Known-occupation denominator", "Logs"),
  extract_irca_post(m_ln_occ_total,  "Full denominator", "Logs"),
  extract_irca_post(m_ln_noocc,      "No-occupation count", "Logs")
) |>
  mutate(
    est = estimate,
...
lvl_order <- c("Blue-collar count","Known-occupation denominator","Full denominator","No-occupation count")
log_order <- c("Blue-collar count","Known-occupation denominator","Full denominator","No-occupation count")

coef_lvl <- coef_df |>
  filter(grp == "Levels") |>
  mutate(outcome = factor(outcome, levels = lvl_order))
...
p_decomp_levels <- ggplot(coef_lvl, aes(x = outcome, y = est)) +
  ...
  labs(
    x = NULL,
    y = "Estimated IRCA × Post effect (counts)",
    title = "Denominator expansion drives the headline share result",
    subtitle = "High-exposure countries show larger gains in denominator counts than in blue-collar counts"
  ) +
  theme_port()
save_plot(p_decomp_levels, "outputs/figures/fig_decomp_levels_only.png", w = 8.2, h = 4.9)

coef_log <- coef_df |>
  filter(grp == "Logs") |>
  mutate(outcome = factor(outcome, levels = log_order))
...
p_decomp_logs <- ggplot(coef_log, aes(x = outcome, y = est)) +
  ...
  labs(
    x = NULL,
    y = "Estimated IRCA × Post effect (log points)",
    title = "The proportional pattern also points to denominator growth",
    subtitle = "Log(1+x) specifications tell a similar story to the levels decomposition"
  ) +
  theme_port()
save_plot(p_decomp_logs, "outputs/figures/fig_decomp_logs_only.png", w = 8.2, h = 4.9)
...
p_sc <- ggplot(chg_cty, aes(x = irca_intensity, y = d_blue)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point() +
  ...
  labs(
    x = "IRCA exposure",
    y = "Δ blue share (89–92 minus 82–85)",
    title = "Exposure vs change in blue-collar share"
  ) +
  theme_port()
...
p1 <- ggplot(dash_df, aes(x=year, y=blue_net, color=irca_group)) +
  ...
  labs(x=NULL, y="Share", title="A. Blue share (net denominator)") +
  theme_port() +
  theme(legend.position = "none")

p2 <- ggplot(dash_df, aes(x=year, y=blue_full, color=irca_group)) +
  geom_line(linewidth=0.6) + geom_point(size=1.3) +
  scale_y_continuous(labels=scales::percent_format(accuracy=1)) +
  labs(x=NULL, y="Share", title="B. Blue share (full denominator)") +
  theme_port() +
  theme(legend.position = "none")

p3 <- ggplot(dash_df, aes(x=year, y=noocc, color=irca_group)) +
  geom_line(linewidth=0.6) + geom_point(size=1.3) +
  scale_y_continuous(labels=scales::percent_format(accuracy=1)) +
  labs(x=NULL, y="Share", title="C. No-occupation share") +
  theme_port() +
  theme(legend.position = "none")

p4 <- ggplot(dash_df, aes(x=year, y=occ_total, color=irca_group)) +
  geom_line(linewidth=0.6) + geom_point(size=1.1) +
  labs(x="Year", y="Mean count", title="D. Full denominator count") +
  theme_port() +
  theme(legend.position = "none")
...
if (!requireNamespace("patchwork", quietly = TRUE)) {
  ...
  save_plot(p1 + theme(legend.position = "none"), "outputs/figures/fig_PORTFOLIO_dash_A.png", w=7, h=4.6)
  save_plot(p2 + theme(legend.position = "none"), "outputs/figures/fig_PORTFOLIO_dash_B.png", w=7, h=4.6)
  save_plot(p3 + theme(legend.position = "none"), "outputs/figures/fig_PORTFOLIO_dash_C.png", w=7, h=4.6)
  save_plot(p4 + theme(legend.position = "none"), "outputs/figures/fig_PORTFOLIO_dash_D.png", w=7, h=4.6)
} else {
  library(patchwork)
  
  if (!requireNamespace("cowplot", quietly = TRUE)) {
    warning("Package 'cowplot' not installed. Falling back to dashboard without shared legend.")
    
    p_dash <- (p1 + p2) / (p3 + p4) +
      patchwork::plot_annotation(
        title = "Dashboard view of the denominator-sensitivity story",
        subtitle = "The shaded band marks the 1989–1992 IRCA surge window"
      )
    ...
    # plot auxiliar solo para extraer una leyenda global
    p_legend_source <- ggplot(dash_df, aes(x=year, y=blue_net, color=irca_group)) +
    ...
    save_plot(p_dash, "outputs/figures/fig_PORTFOLIO_dashboard.png", w=10.5, h=8.4)
  }
}
...
if (length(pre_terms) > 0) {
  w_pre <- suppressWarnings(fixest::wald(es_blue_total, pre_terms))
...
es_coefs <- broom::tidy(es_blue_total) |>
  ...
  mutate(
    ...
    rel_year = year - 1988,
    est_pp   = estimate * 100,
    lo_pp    = (estimate - 1.96*std.error) * 100,
    hi_pp    = (estimate + 1.96*std.error) * 100
  )

p_es <- ggplot(es_coefs, aes(x = rel_year, y = est_pp)) +
  annotate("rect", xmin = 1, xmax = 4, ymin = -Inf, ymax = Inf, fill = "grey70", alpha = 0.15) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_point(size = 2) +
  geom_errorbar(aes(ymin = lo_pp, ymax = hi_pp), width = 0.2) +
  labs(
    x = "Years relative to 1988",
    y = "Effect on blue share (pp)",
    title = "Event-study timing check for the blue-share result",
    subtitle = "The shaded area corresponds to 1989–1992; pre-period movement matters, so the main estimates are descriptive"
  ) +
  theme_port()
save_plot(p_es, "outputs/figures/fig_es_blue_total.png", w=7.2, h=4.8)
...
tidy2 <- bind_rows(
  ...
  tidy(fe_blue_2post_net)   %>% mutate(spec = "Net of adjustments")
) %>%
  filter(grepl("period::(early|late).*irca_intensity|irca_intensity.*period::(early|late)", term)) %>%
  mutate(
    label = ifelse(grepl("early", term), "IRCA window\n(1989–1992)", "Later post-period\n(1993–2000)"),
    est_pp = estimate*100,
    se_pp  = std.error*100,
...
p_early_late <- ggplot(tidy2, aes(x = label, y = est_pp, shape = spec)) +
  ...
  labs(x = NULL, y = "Effect on blue share (pp)",
       title = "Exposure gradient is stronger during the IRCA window") +
  theme_port()
...
p_pre <- ggplot(blue_pre, aes(x=year, y=mean_blue, color=irca_group)) +
  ...
  labs(x="Year", y="Mean blue-collar share",
       title="Pre-IRCA group means already differ somewhat") +
  theme_port()
...
main_coef_plot_df <- bind_rows(
  coef_ci_df(fe_blue_net_cmp, "irca_intensity:post", "Net denominator"),
  coef_ci_df(fe_blue_fullden_cmp, "irca_intensity:post", "Full denominator")
) |>
  mutate(
    estimate_pp = 100 * estimate,
    lo_pp = 100 * lo,
    hi_pp = 100 * hi,
    label = factor(label, levels = c("Net denominator", "Full denominator"))
  )

p_main_compare <- ggplot(main_coef_plot_df, aes(x = label, y = estimate_pp)) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_pointrange(aes(ymin = lo_pp, ymax = hi_pp), size = 0.4) +
  labs(
    x = NULL,
    y = "Estimated IRCA × Post effect on blue share (pp)",
    title = "The headline shrinks sharply under a fuller denominator",
    subtitle = "Same sample comparison: the apparent blue-share decline is much larger under the net denominator"
  ) +
  theme_port()
save_plot(p_main_compare, "outputs/figures/fig_main_net_vs_full_coef.png", w = 7.0, h = 4.5)
...
p_noocc <- ggplot(noocc_trends, aes(x=year, y=mean_noocc, color=irca_group)) +
  ...
  labs(
    x="Year",
    y="Mean no-occupation share",
    title="No-occupation series behaves differently across exposure groups",
    subtitle="Descriptive group means only; use together with the regression evidence rather than as a standalone causal signal"
  ) +
  theme_port()
p_noocc <- add_irca_band(p_noocc)
save_plot(p_noocc, "outputs/figures/fig_noocc_trends_by_exposure.png", w=7.2, h=4.8)
...
p_bins <- ggplot(bin_coefs, aes(x=quartile, y=pp)) +
  ...
  labs(
    x="Exposure quartile",
    y="Post effect on blue share (pp)",
    title="Estimated blue-share associations by exposure quartile",
    subtitle="Descriptive FE comparison; Q1 is the omitted reference group"
  ) +
  theme_port()
save_plot(p_bins, "outputs/figures/fig_bins_blue.png", w=6.5, h=4.2)
...
mech_labels <- c(
  blue_share = "Blue-collar share",
  serv_share = "Services share",
  white_share = "White-collar share",
  fam_share = "Family-based share",
  emp_share = "Employment-based share",
...
coef_df_mech <- bind_rows(lapply(names(mods_mech), function(nm){
  ...
  mutate(
    outcome = recode(outcome, !!!mech_labels),
    pp = 100*estimate,
    lo = 100*(estimate - 1.96*std.error),
    hi = 100*(estimate + 1.96*std.error)
  )
...
p_mech <- ggplot(coef_df_mech, aes(x=reorder(outcome, pp), y=pp)) +
  ...
  labs(
    x=NULL, y="Effect (pp)",
    title="Mechanism scan across related outcomes"
  ) +
  theme_port()
save_plot(p_mech, "outputs/figures/fig_mechanisms_coefplot.png", w=7.2, h=4.9)
...
p_res <- ggplot(res_df, aes(x=year, y=mean_resid)) +
  ...
  labs(x="Year", y="Mean residual",
       title="Residual sanity check: baseline Net") +
  theme_port()
...
loo_res <- bind_rows(loo) |>
  mutate(
    beta_pp = 100 * beta,
    drop = fct_reorder(drop, beta_pp)
  )

p_loo <- ggplot(loo_res, aes(x = drop, y = beta_pp)) +
  geom_hline(yintercept = 100 * b_ref, linetype = "dashed") +
  geom_point() +
  coord_flip() +
  labs(
    x = "Country left out",
    y = "Estimated IRCA × Post effect (pp)",
    title = "The baseline blue-share result is not driven by one country"
  ) +
  theme_port()
save_plot(p_loo, "outputs/figures/fig_loo_blue_net.png", w=7.2, h=5.1)
